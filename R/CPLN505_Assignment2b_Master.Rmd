...
```{r dat example 1}
#dat$resburden<- dat$TOTAL_OFFICIAL_NEED/max(dat$PROJ_RES_REC_COLLCTN, dat$PROJ_RES_REC_TRMT) 
```

mutate -
add field for residential burden (need/ppl)
add field for shortfall (res rec collectn - res_pop)
...
```{r correlation medinc09}
cor(cwns2012$res_pop10_dens, cwns2012$CSS, use="complete.obs", method="pearson")
```
...
```{r}
model1<-lm (TOTAL_OFFICIAL_NEED ~ res_pop10_dens + CSS + MEDINC09, data = cwns2012)
summary(model1)
```

```{r}
model2<-lm (TOTAL_OFFICIAL_NEED ~ res_pop10_dens + POP10 + res_pop_dens_chng_80_10, data = cwns2012)
summary(model2)
```
