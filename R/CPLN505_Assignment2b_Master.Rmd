...
summary(mod4)
```
Interacting 'res_pop10_dens' with 'css' improves the fit but a number of variables lose their significance. What happens if we drop the least significant term? In this case, 'LogPOP10'.

```{r mod4 step}
mod5 <- lm (TOTAL_OFFICIAL_NEED ~ PRES_RES_REC_TRMT + res_pop10_dens:CSS + LogMEDINC09 + GrowingCity,
            data = cwns2012)
summary(mod5)
```
...
ggplot(mod4_resid, aes(x = .fitted, y = .resid)) + 
  ggtitle("Final Regression Model Residuals") +
