...
To model infrastructure need, we need variables that represent it. Based on our tests of correlation and association, we select the following variables to include in our models:
...
* log of official need & log of residential burden  
* rojected change in residences receiving treatment & projected change in residences receiving collection  
* residential population density in 2010 & residential population density change from 1980 to 2010

Residential population density in 2010 and residential burden also seem to be correlated.

```{r correlation matrix}
#make matrix of variables we're testing
mod_vars <- cwns2012 %>% 
  select(TOTAL_OFFICIAL_NEED, LogOfficialNeed, res_pop10_dens, res_pop_dens_chng_80_10, proj_change_res_rec_trmt,
         proj_change_res_rec_collctn, POP10, MEDINC09, ResBurden, LogResBurden, CSS)
...
#plot a correlogram
corrplot(cormatrix, method = "circle", type = "full", order = "hclust", 
...
Testing relationships between need and each of the variables considered above (minus the variables that exhibit strong multi-colinearity with need).
