...
**Need vs PRES_RES_REC_TRMT** appears to be significant and its coefficient suggests a one unit increase residences recieving treatment leads to a $670 increase in need.
...
**Need vs. res_pop10_dens** also appears to be significant and to have a larger effect, per unit increase, on need.
...
Not understanding why but the presence of **CSS** is associated with higher need, based on the intercept, and is statistically significant. However, this it is consistent with our earlier findings.
...
**Need vs. LogPOP10** also appears to be significant and to have a large effect, per unit increase, on need.
...
**PCTWHITE10 vs. Need** shows us there is a large negative relationship between a 1% increase in white population and need.
...
**GrowingCity vs. need** shows that this variable is also significant and growing cities are associated with higher need (based on the intercept).
...
**lowincome vs. need** shows that this variable is also significant and low-income cities are associated with higher need (based on the intercept).
...
**Interpreting the Results of mod1**
The model's R-squared and adjusted R-squared aren't awful, but they could be higher. PCTWHITE10 isn't significant to the model. The p-value associated with the F-statistic is low, which is good. Every other variable appears to be significant. Model 1 could perform better, but seems like a good start.


### Model 2

Now we can employ automated step wise backwards selection on this model to see which, if any variables we can drop. The results tell us we can drop 'PCTWHITE10' because the AIC is lower for the model when it is not included, showing that a model without PCTWHITE10 has a better fit with the data. This gives us model 2.
...
```{r mod3 residuals}
mod3_resid <- augment(mod3)

ggplot(mod3_resid, aes(x = .fitted, y = .resid)) + 
...
```
Backward selection tells us the model's fit is better with low income included than without.


#### Model 4

What if we try interacting terms from model 2?
...
**Test Final Model for multi-colinearity**
...
mod4_resid <- augment(mod4)

ggplot(mod4_resid, aes(x = .fitted, y = .resid)) + 
...
These plots exhibit heteroscedasticity, because the residuals get large as the predicted values move from small to large.

So, we can proceed with confidence that colinearity among these variables is not hindering the model.

We chose this model through a gradual narrowing down of variables in multiple phases.

Describe coefficients (for each 1 unit increase in these variables, we expect the following change in total need).

The interpretations of the results of our final model (mod4) are as follows:
Furthermore, the very low p-values of PRES_RES_REC_TRMT and res_pop10_dens:CSS mean that they both have very high statistical significance - PRES_RES_REC_TRMT and res_pop10_dens:CSS are both good predictors of TOTAL_OFFICIAL_NEED.

* PRES_RES_REC_TRMT
To speak more specifically about the results of this model, all else equal, for each one unit increase in present residential receiving treatment (PRES_RES_REC_TRMT), there is a $560.2 increase in Official Need (TOTAL_OFFICIAL_NEED).

* res_pop10_dens:CSS
Since "CSS" is a binary variable based on the presence or absence of a combined sewer system, the interaction term (res_pop10_dens:CSS) captures the effect of how the relationship between "res_pop10_dens" and "TOTAL_OFFICIAL_NEED" varies depending on whether or not the region has a combined sewer system.

So, all else equal, the effect of population density on "TOTAL_OFFICIAL_NEED" is stronger in regions with a combined sewer system compared to regions without a combined sewer system.

* GrowingCity
The predictor variable GrowingCity may also have some statistical significance, indicated by its somewhat low p-value of 0.0556. This p-value is slightly above the commonly used significance threshold of 0.05. This means that there is a small possibility that this relationship could be due to chance.

This dummy variable is based on whether or not a city is growing. The coefficient of GrowingCity indicated in the results of this model can be interpreted to mean that cities that are growing have a higher "TOTAL_OFFICIAL_NEED" value by approximately $5.3 million, compared to cities that are not growing.

* Overall Model
The r-squared value of this model is 0.3897, which means that the predictor variables explain about 39% of the variability in the response variable (TOTAL_OFFICIAL_NEED).

In summary, the results of this model are of significance to cities with higher than average population density, combined sewers, large numbers of present residences receiving treatment, and cities that are growing. Philly fits all of these descriptors. Furthermore, Philly's population density and residences receiving treatment are projected to increase, and Philly is a growing city with a CSS. This makes clear that Philly's need for water infrastructure investment, while currently quite high, will only continue to increase in the future. Investment in this infrastructure is an absolute MUST!
