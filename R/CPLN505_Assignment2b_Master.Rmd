...
cwns2012 <- cwns2012 %>% 
  ...
  mutate(highincome = ifelse(MEDINC09 >= 75415, 1, 0))
...
mod_vars <- cwns2012 %>% 
  dplyr::select(TOTAL_OFFICIAL_NEED, log_TOTAL_NEED, res_pop10_dens, res_pop_dens_chng_80_10,
                ...
                PCTWHITE10, CSS, GrowingCity, highincome)
...
```
Based on this cor plot, the following indie vars are colinear and therefore will be mutually exclusive in the steps moving forward.
Inclusion of one can only occur at the exclusion of the other.
...
* MEDINC09 & LogMEDINC09 & LogPOP10 & highincome
...
print('High Income')
cor(cwns2012$highincome, cwns2012$TOTAL_OFFICIAL_NEED, use="complete.obs", method="pearson")
```
ResBurden has a correlation of 1 with Total Official Need, meaning Total Official Need is a function of ResBurden. 
We will therefore exclude ResBurden from our regression analysis moving forward.
...
Based on correlation with Total Official Need, and elimination of one of a variable pair when colinearity is present, favoring the one with higher correlation with need, we narrowed down this list of variables to 6 indie vars.
...
Testing relationships between TOTAL_OFFICIAL_NEED and each of the variables considered above.
...
Scatter plots for each of these variables
...
Now we'll see how those variables can give us a model of need through Multivariate Regression
...
mod1 <- lm (TOTAL_OFFICIAL_NEED ~ PRES_RES_REC_TRMT + res_pop10_dens + CSS + LogPOP10 + PCTWHITE10 + LogMEDINC09, data = cwns2012)
summary(mod1)
...
R-squared and adjusted R-squared aren't awful but they could be higher. PCTWHITE10 doesn't seem to be significant. F-statistic p-value is low. Every other variable appears to be significant. Model 1 could perform better, but seems like a good start.

**Test mod1 for multi-colinearity**
The VIF for Model 1 reveals that that the model's variables do not exhibit multi-colinearity. The VIF values for all the model's variables are below 5 and their square root VIF values are below 2.
...
sqrt(vif(mod1))
```
So, we can proceed with confidence that colinearity among these variables is not hindering the model.

#### Model 2

Now we can proceed with backwards selection on this model.
...
```
The AIC is lower for the model without PCTWHITE10, showing that a model without PCTWHITE10 has a better fit with the data.

and the Analysis of Variance Table shows an ???increase in F statistic???, we can confidently proceed with model2, dropping PCTWHITE10.

```{r mod 2 anova}
anova(mod1, mod2)
```
The large p-value associated with model 2 suggests it does not perform better than model 1.

```{r mod 2 summary}
summary(mod2)
```
Dropping PCTWHITE10 also did not change the model's r-squared at all.


#### Model 3

What if we replace median income with our binary high income variable?

```{r mod 3}
mod3 <- lm (TOTAL_OFFICIAL_NEED ~ PRES_RES_REC_TRMT + res_pop10_dens + CSS + LogPOP10 + highincome, data = cwns2012)
summary(mod3)
```
The r-squared improves slightly.

```{r multi-colinearity test mod3}
print('VIF')
vif(mod3) # variance inflation factors 
print('sqrt(VIF)')
sqrt(vif(mod3))
```
And we've avoided co-linearity.
...
```
Backward selection tells us the model's fit is better with high income included than without.

```{r mod 3 anova}
anova(mod1, mod2, mod3)
```
Anova suggests there is not a statistically meaningful difference between models 1 and 3 (model 2 is the least significant) but the slightly higher r-squared suggests model 3 is the best yet.

#### Model 4

What if we try interacting terms?

```{r mod 4}
mod4 <- lm (TOTAL_OFFICIAL_NEED ~ PRES_RES_REC_TRMT + res_pop10_dens + CSS + LogPOP10 + highincome,
            data = cwns2012)
summary(mod4)
```
...
We chose this model through a gradual narrowing down of variables...
